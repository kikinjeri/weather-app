...
async function loadCrypto(){
    try{
        ...
        // Bitcoin & Ethereum first, then sort by price descending
        data.sort((a,b)=>{
            if(a.name==='Bitcoin') return -1;
            if(b.name==='Bitcoin') return 1;
        ...
        console.error(e);
        $('crypto-container').textContent='Crypto unavailable';
    }
}
...
async function loadSection(category, containerId){
    try{
        ...
        if(category==='sports' && items.length>4){
            const rollUp = document.createElement('div');
            rollUp.className='rollup-link';
            rollUp.innerHTML=`<a href="#">+${items.length-4} more articles</a>`;
            rollUp.onclick=e=>{
                e.preventDefault();
                grid.innerHTML='';
                items.forEach(a=>{
            ...
            grid.appendChild(rollUp);
        }

    }catch(e){
        console.error(e);
        $(containerId).innerHTML='<p>Unable to load news</p>';
    }
}
