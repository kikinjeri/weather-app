...
app.get('/api/weather', async (req,res)=>{
    ...
    } catch(e){ console.error(e); res.status(500).json({error:'Weather API error'}); }
});
...
app.get('/api/crypto', async (req,res)=>{
    try{
        ...
        res.json(formatted);
    }catch(e){ console.error(e); res.status(500).json({error:'Crypto API error'}); }
});
...
const feeds = {
    ...
    sports: ["https://www.espn.com/espn/rss/news","https://www.cbssports.com/rss/headlines/"]
};
app.get('/api/news', async (req,res)=>{
    ...
    try{
        ...
        for(const url of feedUrls){
            ...
            }catch(e){ console.error(`Failed to load feed ${url}:`, e); }
        }
        if(items.length===0) items.push({title:`${cat} feed unavailable`, pubDate:Date.now(), contentSnippet:''});
        items.sort((a,b)=>new Date(b.pubDate)-new Date(a.pubDate));
        res.json({items: items.slice(0,10)});
    }catch(e){ console.error('News API error', e); res.status(500).json({error:'News error'}); }
});
...
app.get('/api/social', async (req,res)=>{
    ...
    try{
        ...
        if(items.length===0) items.push({title:'No social feed available', contentSnippet:'', pubDate:Date.now(), handle:'Source'});
        items.sort((a,b)=>new Date(b.pubDate)-new Date(a.pubDate));
        res.json({items: items.slice(0,20)});
    }catch(e){ console.error('Social feed error', e); res.status(500).json({error:'Social feed error'}); }
});

// STOCKS
app.get('/api/stocks', async (req,res)=>{
    const symbols = ['AAPL','MSFT','TSLA','GOOGL','AMZN','^GSPC','^GSPTSE'];
    const API_KEY = process.env.FINNHUB_API_KEY;
    try{
        const results = await Promise.all(symbols.map(async sym=>{
            const r = await fetch(`https://finnhub.io/api/v1/quote?symbol=${encodeURIComponent(sym)}&token=${API_KEY}`);
            const data = await r.json();
            return {
                symbol: sym,
                price: data.c ? `$${data.c.toFixed(2)}` : 'N/A',
                change: data.dp != null ? data.dp.toFixed(2) : '0'
            };
        }));
        res.json(results);
    }catch(e){ console.error('Stock data error', e); res.status(500).json([]); }
});
